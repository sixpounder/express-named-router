...
var namedRouter = function namedRouter() {
    ...
    /**
     * Expose routes as public
     */
    this.routes = _routes;

    /**
     * this.route(name) is equivalent for this.routes.<name>Path()
     */
    ...
    var namedRouteResolver = function(path,routeOptions) {
        ...
        // Clean remaining options
        var _remainingCheck = new RegExp(":[a-zA-Z0-9_]+\\??", "i");
        p = p.replace(_remainingCheck, "");
    ...
    var addNamedRouteToAppLocals = function(routeName, path) {
        _routes[routeName + "Path"] = function(routeOptions) {
            return namedRouteResolver(path, routeOptions);
        };

        var installPoint = appHelpersInstallPoint();

        installPoint[routeName + "Path"] = _routes[routeName + "Path"];
